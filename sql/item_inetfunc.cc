...
class Inet4
{
  ...
  bool str_to_ipv4(const char *str, size_t str_length, CHARSET_INFO *cs);
public:
  // Initialize from a text representation
  Inet4(bool *error, const char *str, size_t length, CHARSET_INFO *cs)
  {
    *error= !str_to_ipv4(str, length, cs);
  }
  // Initialize from a binary representation
  Inet4(bool *error, const char *str, size_t length)
  {
...
Inet6::Inet6(bool *error, Item *item)
{
  ...
  */
  if (!(*error= (!str || str->length() != sizeof(m_buffer) ||
